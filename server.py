...
class PromptServer():
    ...
    def schedule_image_deletion(self, prompt_id):
        """Schedule deletion of images generated by this prompt after 1 seconds"""
        import time
        import os

        # Record the current time and output directory state
        ...
        try:
            ...
            logging.warning(f"Error scanning output directory: {e}")

        self.pending_deletions[prompt_id] = (existing_files, current_time)
        logging.info(f"Scheduled image deletion for prompt {prompt_id}")

    def cleanup_generated_images(self, prompt_id):
        """Delete images that were generated after the prompt was submitted"""
        if prompt_id not in self.pending_deletions:
            return

        import os

        existing_files, start_time = self.pending_deletions.pop(prompt_id)
        output_dir = folder_paths.get_output_directory()

        # List all current files before deletion
        current_files = []
        try:
            for root, dirs, files in os.walk(output_dir):
                for file in files:
            ...
            logging.error(f"Error listing files before cleanup for prompt {prompt_id}: {e}")
            return

        logging.info(f"Starting cleanup for prompt {prompt_id}: found {len(current_files)} total image files")

        # Identify files to delete
        files_to_delete = []
        for full_path in current_files:
            # Skip files that existed before prompt execution
            if full_path in existing_files:
                continue

            # Check if file was created after prompt submission
            try:
        ...
        logging.info(f"Identified {len(files_to_delete)} files to delete for prompt {prompt_id}")
        ...
        for full_path in files_to_delete:
            try:
                os.remove(full_path)
                deleted_count += 1
                logging.info(f"Deleted generated image: {full_path}")
            except (OSError, FileNotFoundError) as e:
                failed_deletions.append(full_path)
                logging.warning(f"Could not delete file {full_path}: {e}")
        ...
        try:
            ...
            logging.error(f"Error listing files after cleanup for prompt {prompt_id}: {e}")

        logging.info(f"Cleanup completed for prompt {prompt_id}: deleted {deleted_count} images, {len(failed_deletions)} failed, {len(remaining_files)} files remaining")

        if failed_deletions:
            logging.warning(f"Failed to delete {len(failed_deletions)} files for prompt {prompt_id}: {failed_deletions}")

    async def delayed_image_cleanup(self, prompt_id):
        """Execute image cleanup after 1 second delay"""
        await asyncio.sleep(1)
        self.cleanup_generated_images(prompt_id)
